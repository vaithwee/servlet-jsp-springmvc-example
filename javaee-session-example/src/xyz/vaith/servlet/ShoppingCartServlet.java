...
import xyz.vaith.bean.ShoppingItem;
...
public class ShoppingCartServlet extends HttpServlet {
    ...
    private NumberFormat currencyFormat = NumberFormat.getCurrencyInstance(Locale.CHINESE);


    @Override
    public void init() throws ServletException {
        products.add(new Product(1, "Bravo 32' HDTV", "Low-cost HDTV from renowned TV manufacturer", 159.59f));
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (uri.equals("/products")) {
            sendProductList(response);
        } else if (uri.equals("/viewProductDetails")) {
            sendProductDetails(request, response);
        } else if ("/viewCart".equals(uri)) {
    ...
    private void showCart(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        writer.println("<html><head><title>Shopping Cart</title></head><body><a href='products'>Products List</a>");
        HttpSession session = request.getSession();
        List<ShoppingItem> cart = (List<ShoppingItem>) session.getAttribute(CART_ATTRIBUTE);
        if (cart != null) {
            writer.println("<table>");
            writer.println("<tr><td style='width: 150px;'>Quantity</td><td style='width: 150px;'>Product</td><td style='width: 150px;'>Price</td><td>Amount</td></tr>");
            double total = 0.0;
            ...
            writer.println("<tr><td colsapn='4' style='text-align:right;'>Total:"+ currencyFormat.format(total)+"</td></tr>");
            writer.println("</table>");
        }
    ...
    private void sendProductDetails(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (product != null) {
            writer.println("<html><head><title>Product Details</title></head><body><h2>Product Details</h2><form method='post' action='addToCart'><input type='hidden' name='id' value='"+productId +"' /><table><tr><td>Name:</td><td>" +product.getName() + "</td></tr><tr><td>Description</td><td>"+product.getDescription() +"</td></tr><tr><td><input type='text' name='quantity' /></td><td><input type='submit' value='Buy'/></td></tr><tr><td colspan='2'><a href='products'>Product List</a></td></tr></table></form></body></html>");
        } else {
    ...
    private void sendProductList(HttpServletResponse response) throws IOException {
        response.setContentType("text/html;charset=utf-8");
        PrintWriter writer = response.getWriter();
        writer.println("<html><head><title>Products</title></head><body><h2>");
        writer.println("<ul>");
        for (Product product : products) {
